...
export function nextStockQty(current: number, signedQty: number, trackStock: boolean): number {
  ...
  if (trackStock && next < -0.0001) {
    throw new AppError("Estoque insuficiente para este produto.", 409, undefined, "INSUFFICIENT_STOCK");
  }
...
export function planRetailSaleLines(
  ...
  if (!items.length) {
    throw new AppError("Informe ao menos um produto", 400);
  }
  ...
  return [...merged.entries()].map(([productId, item]) => {
    ...
    if (!product.active) {
      throw new AppError(`"${product.name}" está inativo e não pode ser vendido`, 400);
    }
    if (product.type === "CONSUMABLE") {
      throw new AppError(`"${product.name}" é de uso interno e não pode ser vendido`, 400);
    }
